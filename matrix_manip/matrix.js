...
Matrix.prototype = {
	...
	setTranslation: function(x,y) {
		...
		{
			let p = x
			this.setTransform(this.a, this.b, this.c, this.d, p.v.x, p.v.y);
		}
		else
		{
	...
	setScale: function(x,y) {
		...
		if (arguments.length === 1)
		{
			...
			vX.mult(scale)
			vY.mult(scale)
			this.setTransform(vX.v.x, vX.v.y, vY.v.x, vY.v.y, this.e, this.f);
		}
		else
		{
			vX.mult(x)
	...
	getMult: function(m) {
		...
		local.mult(m)
		//let p = m.get_row(2)
		//p = p5.Vector.add( p, p5.Vector.mult( m.get_row(0), local.x))
		//p = p5.Vector.add( p, p5.Vector.mult( m.get_row(1), local.y))
